use crate::models::tracforce_post_model::ConfigFile;
use crate::{DESKTOP_MACADD_DATA, SHARED_PAYLOAD};
...
pub async fn start_application_threads(config: ConfigFile, shared_payload: Arc<Mutex<String>>) {
    // println!("{:?}", config);
    ...
    tokio::spawn(run_periodic_task(100, {
        ...
        move || {
            ...
            async move {
                let payload = shared_payload.lock().unwrap().clone();
                if let Err(e) = detect_console_command(5, &payload).await {
                    log_error_fn(&format!("Console command detection failed: {:?}", e));
                }
            }
        }
    }));
        ...
        move || {
            ...
            async move {
                ...
                if let Err(e) = get_search_key_from_url_fn(90, payload).await {
                    println!("Failed to get search key from URL: {:?}", e);
                    log_error_fn(&format!(
                        "Failed to get Search from Chrome history: {:?}",
                        e
                    ));
                }
            }
    ...
    tokio::spawn(run_periodic_task(1800, {
        #[cfg(any(target_os = "linux", target_os = "macos"))]
...
pub async fn run_with_dynamic_config() {
    let shared_payload = Arc::new(Mutex::new(SHARED_PAYLOAD.lock().unwrap().clone()));

    match get_mac_address() {
        Ok(Some(ma)) => {
            let mut mac_address_data = DESKTOP_MACADD_DATA.lock().unwrap();
            *mac_address_data = ma.to_string();
            println!("MAC addr = {}", ma);
        }
        Ok(None) => {
            println!("No MAC address found.");
            log_error_fn("No MAC address found.");
        }
        Err(e) => {
            println!("{:?}", e);
            log_error_fn(&format!("Failed to get MAC address: {:?}", e));
        }
    };
        ...
        match fetch_configuration_from_api().await {
            Ok(config) => {
                ...
                handle = Some(tokio::spawn(async move {
                    start_application_threads(config, shared_payload).await;
                }));
            }
            Err(e) => {
                log_error_fn(&format!("Failed to fetch configuration: {:?}", e));
            }
        }
